...
class StockSquickMove(http.Controller):
    ...
    def quickmove_location_search(self, **post):
        ...
        return werkzeug.wrappers.Response(status='200', content_type="application/json; charset=utf-8",
                                          response=json.dumps({'results': results}, indent=4, sort_keys=True))
    ...
    def quickmove_get_product_stock(self, location_id=None, product_id=None, **kw):
        ...
        for product in products:
            ...
            reserved_qty = sum(quants.mapped(lambda q: q.reserved_quantity))
            # unreserved_qty = sum(quants.filtered(lambda q: False if q.reservation_id else True).mapped('qty'))
            unreserved_qty = sum(quants.mapped(lambda q: q.available_quantity))
    ...
    def quickmove_inventory(self, product=None, **post):
        return request.render('stock_quickmove.webapp_inventory', {'title': 'Inventory', 'product' :product, 'inv_class': 'active'})
